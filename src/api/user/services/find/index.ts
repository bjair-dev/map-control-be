...
export const findUserByEmail = async ({ email }: { email: string }): Promise<UserAttributes> => {
  try {
    return (
...
export const findUserByEmailWithoutState = async ({ email }: { email: string }): Promise<UserAttributes> => {
  try {
    return (
      await DataBase.instance.user.findOne({
        where: {
...
export const findUserByDni = async ({ dni }: { dni: number }): Promise<UserAttributes> => {
  try {
    return (
      await DataBase.instance.user.findOne({
        where: {
  ...
  try {
    const listRangeAge = await DataBase.instance.user.findAll({
      attributes: [
        ...
        WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 70 AND 79 THEN 'VII) 70-79' ELSE 'VIII) 80>' END`), 'range_age'],
        ...
        WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 30 AND 39 THEN 'III) 30-39' WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 40 AND 49 THEN 'IV) 40-49' 
        WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 50 AND 59 THEN 'V) 50-59' WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 60 AND 69 THEN 'VI) 60-69' 
        WHEN TIMESTAMPDIFF(YEAR, date_of_birth , CURRENT_TIMESTAMP()) BETWEEN 70 AND 79 THEN 'VII) 70-79' ELSE 'VIII) 80>' END)`), 'cantidad'],
