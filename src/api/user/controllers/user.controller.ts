...
import { findAllUsers, findUserById, findAllUsersRecordType, findAllUsersRangeAge, SearchUser } from '../services/find/index'
...
export const findAllUsersRecordTypeController = async (
  ...
  try {
    ...
    // if( copy_list.length > 0 ){
      if(!(copy_list.some(( item:any )=> item.origin == 'google')))copy_list = [...copy_list,{origin:'google',quantity:0 }];
      if(!(copy_list.some(( item:any )=> item.origin == 'facebook')))copy_list = [...copy_list,{origin:'facebook',quantity:0 }];
      if(!(copy_list.some(( item:any )=> item.origin == "correo")))copy_list = [...copy_list,{origin:'correo',quantity:0 }];
...
export const findAllUsersRangeAgeController = async (
  ...
  try {
    ...
    const data  = [...menor_20,...de20_29,...de30_39,...de40_49,...de50_59,...de60_69,...de70_79,...mayor80]
...
export const findProfileUserController = async (
  ...
  try {
    ...
    const profileUser = await findUserById({ id:userId ,attributes:['terms_and_conditions','name','lastname','sexo','dni','email','date_of_birth','nightmode','origin','number_of_sessions']})
...
export const udpateDaySessionUserController = async (
  ...
  try {
    ...
    const profileUser = await findUserById({ id:userId ,attributes:['user_session_day','date_user_session_day']})
    ...
    if(profileUser?.date_user_session_day?.toString() != convertToday && profileUser?.date_user_session_day != null ){
