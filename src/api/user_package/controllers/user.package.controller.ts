...
import { checkDayOnePackageForUser, CountPackageUser, SearchLatestPackage, SearchPackage, SearchPackageTemporary, SearchPackageTemporaryStatusFinished, SearchUserPackagesStatusFinished } from '../services/find/user.package'
...
export const getContentForUserController = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
        ...
        if(answeredQuestion21 < count_question ){
          ...
          if(packageTemporaryPending){
              ...
              return res.status(200).json(Object.assign({ type:'temporary',answeredQuestion:answeredQuestion21 },getContentPending))
          }
        ...
        if(packagePending){
            ...
            return res.status(200).json(Object.assign({ answeredQuestion:answeredQuestion21 },getContentPending))
        ...
        if(!packagePending && !packageTemporaryPending){
          ...
          const latestUserPackage = await DataBase.instance.userPackage.findOne({
            ...
            if(latestUserPackage){
              ...
              daySession = latestUserPackage?.date == currentDate ? Number(latestUserPackage?.user_session_day) : Number(latestUserPackage?.user_session_day)+1
            }
            ...
            if(FinishedPackages.count>=packagesByDateOrSession.length){
              ...
              //Falta ver mas paquetes
              let temporaryUserSession = await DataBase.instance.temporaryUserSession.findOne({where:{userId}})
        ...
        if(answeredQuestion21 >= count_question ){
          ...
          if(FinishedPackages.count >= packagesByDateOrSession.length){
            return res.json("Ya no tienes contenido para ver hoy dia")
          }
            ...
            if(latestUserPackage?.date != convertToday){
              ...
              if(packagesByDateOrSession.length == 0)return res.json("No hay contenido programado para hoy")
            ...
            if(FinishedPackages.count<packagesByDateOrSession.length){
              ...
              const getRandomIdMissing = outPackageBySessionDayIds.length>0 ?outPackageBySessionDayIds[0]: idsMissing[0]
  ...
  export const updateContentForUserController = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
  ...
  export const updateIndexUserPackageController = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
  ...
  export const getHistoryOfTipsController = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const tips = await FilterTips(id_tips,Number(tip_category_id))
  ...
  export const getHistoryOfVideosController = async (
    req: Request,
    res: Response,
    next: NextFunction
  ) => {
    try {
      ...
      const videos = await FilterVideos(id_videos)
