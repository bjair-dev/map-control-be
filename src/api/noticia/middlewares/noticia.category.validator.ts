...
} from '../validator/noticia.category.custom'
...
export const updateTipCategoryValidator = [
  param('id').isNumeric().withMessage('Se require un numero').bail().custom(existsTipCategory),
  body('category')
    .isString()
    .withMessage('Se require un string')
    .bail()
...
export const findTipCategoryValidator = [
  param('tip_category_id').isNumeric().withMessage('Se require un numero').bail().custom(existsTipCategory),
  allValidator,
]
