...
import {
  ...
  findAllNotificationsOutPagination,
  findContentTypeNotification,
...
export const notificationForUserController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const notificationsSuccessfull = await DataBase.instance.userNotification.findAll({
      where: {
        userId,
        date: date_local,
        status: 'terminado',
    ...
    const filterIdNotificationsSuccessfull:any[] = notificationsSuccessfull.map( item => item.notificationId)
    ...
    if (latestUserNotificationsToday.length > 0) {
      ...
      for (const item of copy_notifications) {
        let userNotificationId = latestUserNotificationsToday.find(
          (element) => element?.notificationId == item?.id
        )
        let content: any = await findContentTypeNotification(
          Number(item?.content_id),
          Number(item?.content_type)
        )
        let notificationId = item?.id
        delete item['id']
        data = [
          ...data,
          Object.assign(content, item, { notificationId, userNotificationId: userNotificationId?.id }),
        ]
    ...
    // if (countNotificationsGlobal != 0) {
      ...
      const possibleNotifications: NotificationAttributes[] = await DataBase.instance.notification.findAll({
        where: {
          state: true,
          id:filterIdNotificationsSuccessfull.length>0?{
            [Op.notIn]:filterIdNotificationsSuccessfull
          }:{[Op.gt]:0},
          [Op.or]: [
            {
              [Op.and]: {
      ...
      // return res.json(possibleNotifications)
      //Si no hay posibles notificaciones para el dia de hoy o por dia de sesion retornara - notification_completed
     if(possibleNotifications.length == 0) return res.status(200).json({ status:'notification_completed' })
    //  console.log('Si hay posibles notificaciones')
      //Estan terminado todas las notificaciones
    // if (countNotificationsTerminado != 0 && countNotificationsTerminado == countNotificationsGlobal)
    // console.log(possibleNotifications.length,"possibleNotifications length" , countNotificationsGlobal,countNotificationsTerminado)
    
    // if (possibleNotifications.length == 0 || countNotificationsTerminado == possibleNotifications.length)return res.status(200).json({ status: 'notification_completed' })
    for (const item of possibleNotifications) {
        const filters: Array<any> = JSON.parse(item.filtros as string)
        const outQuestionId: any = filters.map((item: any) => {
          if (item.active == true) return item.questionId
        })
        const outQuestionValue: any = filters.map((item: any) => {
          if (item.active == true) return item.value
        })

        const other_filters: Array<any> = JSON.parse(item.filtros_opcionales as string)
        const outActiveOtherFilters: Array<any> = other_filters.filter((item: any) => {
        ...
        outActiveOtherFilters.forEach((item) => {
          if (item?.tag == 'Edad') {
            ...
            if (
              ...
              item?.value?.desde == years_of_user
            ...
            if (
              ...
              item?.value?.desde != item?.value?.hasta &&
              item?.value?.desde <= years_of_user &&
              item?.value?.hasta >= years_of_user
            ) {validate_other_filters.push(1)}
        ...
        if (filters.length > 0) {
          ...
          if (
            findFilter.length == outQuestionId.length &&
            validate_other_filters.length == outActiveOtherFilters.length
          ) {
    ...
    if (data_filters.length > 0) {
      ...
      for (const item of copy_notifications) {
        ...
        data = [
          ...data,
          Object.assign(content, item, { notificationId, userNotificationId: item?.userNotificationId }),
        ]
    ...
    // if(data_filters.length == 0)console.log("si tienes posibles notificaciones pero ninguno coincide")
    if(data_filters.length == 0) return res.status(200).json({ status:'notification_completed' })
...
export const changeStatusUserNotificationController = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
